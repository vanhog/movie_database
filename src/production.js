...
document.getElementById('up')?.addEventListener('input', () => {
  ...
  let viewList = movies.sort((a, b) => Number(a[1]) - Number(b[1]));
  populateList(viewList);
});

document.getElementById('down')?.addEventListener('input', () => {
  let viewList = movies.sort((a, b) => Number(b[1]) - Number(a[1]));
  populateList(viewList);
});

document.getElementById('rate')?.addEventListener('input', () => {
  let viewList = movies.sort((a, b) => Number(b[5]) - Number(a[5]));
  populateList(viewList);
});
